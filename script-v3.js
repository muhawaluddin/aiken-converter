document.addEventListener('DOMContentLoaded', function() {
    ...
    const conversionStats = document.getElementById('conversionStats') || createConversionStatsElement();
    
    // Create conversion stats element if it doesn't exist
    function createConversionStatsElement() {
        const statsElement = document.createElement('div');
        statsElement.id = 'conversionStats';
        statsElement.className = 'stats-container';
        statsElement.style.marginBottom = '15px';
        statsElement.style.padding = '10px';
        statsElement.style.backgroundColor = '#f8f9fa';
        statsElement.style.borderRadius = '5px';
        statsElement.style.border = '1px solid #dee2e6';
        
        // Insert before resultCard
        const resultCardElement = document.getElementById('resultCard');
        resultCardElement.parentNode.insertBefore(statsElement, resultCardElement);
    ...
    convertBtn.addEventListener('click', () => {
        ...
        const conversionResult = convertToAiken(input);
        outputText.value = conversionResult.formattedText;
        
        // Show conversion stats
        const totalInputQuestions = conversionResult.totalInputted;
        const successfullyConverted = conversionResult.successfullyConverted;
        
        conversionStats.innerHTML = `
            <div style="font-weight: bold; margin-bottom: 5px;">Ringkasan Konversi:</div>
            <div>Jumlah soal terdeteksi: ${totalInputQuestions}</div>
            <div>Jumlah soal berhasil dikonversi: ${successfullyConverted}</div>
            ${totalInputQuestions !== successfullyConverted ? 
                `<div style="color: #dc3545; margin-top: 5px;">
                    <span style="font-weight: bold;">Hint:</span> Beberapa soal tidak memiliki kunci jawaban atau format tidak sesuai.
                    Pastikan setiap soal memiliki minimal 2 opsi dan kunci jawaban (contoh: "JAWABAN: A").
                </div>` : 
                '<div style="color: #28a745; margin-top: 5px;">Semua soal berhasil dikonversi!</div>'
            }
        `;
        conversionStats.style.display = 'block';
        
        // Generate preview
        generatePreview(conversionResult.formattedText);
        
        resultCard.style.display = 'block';
        
        // Scroll to results
        conversionStats.scrollIntoView({ behavior: 'smooth' });
    });
    ...
    function convertToAiken(input) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!collectingQuestion && !line.match(/^[A-Za-z][.)]\s/) && !line.toLowerCase().startsWith('jawaban:') && !line.toLowerCase().startsWith('answer:')) {
                // If we find a new question, count the potential previous one
                if (currentQuestion) {
            ...
            if (collectingQuestion) {
                ...
                if (line === '' || answerMatch || i === lines.length - 1) {
                    // If last line of file and not empty, we need to make sure we count this option
                    if (i === lines.length - 1 && line !== '' && !answerMatch) {
                        const lastLineOptionMatch = line.match(/^([A-Za-z])[.)]\s+(.+)$/);
                        if (lastLineOptionMatch) {
                            currentOptions.push({
                                letter: lastLineOptionMatch[1].toUpperCase(),
                                text: lastLineOptionMatch[2]
                            });
                        }
                    }
        ...
        return {
            ...
            totalInputted: totalQuestionsDetected,
            successfullyConverted: questions.length,
            potentialIssues: potentialQuestions.filter(q => q.optionsCount < 2 || !q.hasAnswer)
        };
    }
    ...
    function generatePreview(aikenText) {
        ...
        // If no questions were successfully converted
        if (!aikenText.trim()) {
            previewContainer.innerHTML = '<div class="empty-preview">Tidak ada soal yang berhasil dikonversi. Periksa format input Anda.</div>';
            return;
        }
