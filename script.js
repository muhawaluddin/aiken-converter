document.addEventListener('DOMContentLoaded', function() {
    ...
    function convertToAiken(input) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!collectingQuestion && !line.match(/^[A-Z][.)]\s/) && !line.toLowerCase().startsWith('jawaban:') && !line.toLowerCase().startsWith('answer:')) {
            ...
            if (collectingQuestion) {
                ...
                const answerMatch = line.match(/^(ANS|jawaban|answer):?\s*([A-Z])[.)]*\s*$/i);
                if (answerMatch) {
